...
public class QueryRecordService extends IntentService {
    ...
    private QueryBuilder<Record> fetchBuilder(String fromDate, String toDate, String sex, String result) {
        ...
        if ("成功".equals(result)) {
            builder.whereOr(RecordDao.Properties.Status.eq("人脸通过"), RecordDao.Properties.Status.eq("指纹通过"), RecordDao.Properties.Status.eq("人脸、指纹通过"));
        } else if ("失败".equals(result)) {
            builder.whereOr(RecordDao.Properties.Status.eq("人脸不通过"), RecordDao.Properties.Status.eq("指纹不通过"), RecordDao.Properties.Status.eq("白名单检验失败"));
        }
